"""Shared plumbing: config, paths, logging, HTTP, run bookkeeping.

Deliberately the same shape as quantaura-youtube's common.py — anyone who has
read one pipeline has read them both.
"""
...
def get_logger(name: str) -> logging.Logger:
    ...
    # Windows consoles often run cp1252, which cannot encode arrows/dashes.
    try:
...
class PipelineAbort(Exception):
    """Raised when today's post cannot be produced safely.

    The orchestrator catches this, notifies, and exits cleanly. Skipping a day
    is a normal outcome — posting something unverified is not.
    """
...
def http_session() -> requests.Session:
    ...
    if _session is None:
        ...
        _session.headers.update({"User-Agent": "QuantAura-X/1.0 (+https://quantaura.tech)"})
    return _session
...
def get_json(url: str, *, params: dict | None = None, headers: dict | None = None,
    ...
    decides whether a given source is fatal or merely degrades the post."""
    cfg = config()["api"]
...
# ── Run bookkeeping ───────────────────────────────────────────────────────────
# Committed after every run, same convention as the video pipeline: the whole
# operational history reads in one place, and the catch-up cron uses it to
# know the day is already done.

RUNS_PATH = STATE_DIR / "runs.jsonl"


def posted_today() -> bool:
    """Has a post already gone out for today's UTC date?

    The schedule runs twice (13:30 and a 16:30 catch-up) because GitHub's cron
    is best-effort and drops runs under load. Two chances at posting only help
    if the second is a no-op when the first succeeded — this is that guard.
    """
    ...
    for line in RUNS_PATH.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        try:
            ...
            continue
        if row.get("date") == today and row.get("status") == "posted":
            return True
    return False


def record_run(status: str, stage: str, detail: str, extra: dict | None = None) -> None:
    from datetime import datetime, timezone
    row = {
